...
(function (root, jQuery) {
    var $ = jQuery;
    jQuery.fn.kirkiSlider = function (options) {
        var self = this;

        this.find( 'input[type=range]' ).on( 'mousedown', function() {
            value = jQuery( this ).attr( 'value' );
            jQuery( this ).mousemove( function() {
                value = jQuery( this ).attr( 'value' );
                self.find( '.kirki_range_value .value' ).text( value );
            });
        });

        // Handle the reset button
        this.find( '.kirki-slider-reset' ).click( function() {
            thisInput    = jQuery( this ).closest( 'label' ).find( 'input' );
            inputDefault = thisInput.data( 'reset_value' );
            thisInput.val( inputDefault );
            thisInput.change();
            self.find( '.kirki_range_value .value' ).text( inputDefault );
        });

        this.find('input').on("change", function() {
           options.onChange(jQuery(this).val());
        });
...
(function (root, $) {
    var ColorStopItemView = Backbone.View.extend({
        className: 'list-item',
        template: _.template('<input type="text" data-alpha="true" value="<%= color %>" class="color-picker"/>'),
        ...
        initialize: function() {
            ...
            options.change = function (event, ui) {
                self.model.set("color", ui.color.toString());
            };
        ...
        render: function() {
            var colorPicker = this.$el.find(".color-picker");
            colorPicker.val(this.model.color);
        },
    ...
    wp.customize.controlConstructor['gradient-control-pro'] = wp.customize.Control.extend({
        ...
        ready: function () {
            ...
            var colorStops = new Backbone.Collection(val.colors);

            var gradient = new Backbone.Model({
                initialize: function () {
                    this.set("colors", colorStops);
                    this.set("alpha", 0);
                }
            });


            var list = new Backbone.ListView({
                collection: colorStops,
                itemView: ColorStopItemView,
                el : this.container.find('.colors')
            });

            list.render();

            list.on('item:destroy', function(itemView) {
                colorStops.remove(itemView.model);
            });

            colorStops.on('change', function() {
                console.warn('list change');
                console.warn(colorStops);

                var setting = CP_Customizer.utils.getValue(control);
                setting.colors = colorStops.toJSON();
                control.setValue(setting, true);
            });
            ...
            this.container.find('.angle').knob({
                fgColor : "#AAAAAA",
                bgColor : "#FFFFFF",
                width:80,
                height:80,
            ...
            this.container.find('.customize-control-kirki-slider').kirkiSlider({
                onChange : function(val) {
                    var setting = CP_Customizer.utils.getValue(control);
                    setting.colors[0].position = (val * 100 + "%");
                }
            });
        ...
        setValue: function (value, silent) {
            ...
            this.update(value, silent);
        },

        update: function(value, silent) {
            if (!silent) {
                this.colorStops.set(value.colors);
            }
            this.container.find('.angle').val(value.angle).trigger('change');
        ...
        setColors: function (colors) {
            var setting = CP_Customizer.utils.getValue(this);
            setting.colors = colors;
            this.setting.set(JSON.stringify(setting));
            this.colorStops.set(colors);
        },
